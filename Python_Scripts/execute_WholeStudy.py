...
solAlg = [1, 2]
maxStep = 10000
study_indicator = 1                                         # For the main study
    ...
    for iNonLin in trange(0, len(iter)):
        for iLinSol in trange(0, len(linSol)):
            for iToleranceTupel in trange(0, len(atol)):
                simulate(atol[iToleranceTupel], rtol[iToleranceTupel], linSol[iLinSol], iter[iNonLin], solAlg[iSolALg], maxStep, study_indicator)
