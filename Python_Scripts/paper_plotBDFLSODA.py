...
for iFile in range(0, int(len(list_directory_general)/2)):
    ...
    if bdf_split[0] == '2' and bdf_split[1] == '2'and bdf_split[2] == '9':
...
ratio_bdf_zero = []
ratio_equal_zero = []
num_states_lsoda_bdf = []
num_states_bdf_lsoda = []
num_states_equal = []
...
for iTsvFile in range(0, len(list_directory_lsoda)):
    ...
    for iModel in range(0, len(lsoda_tsv_file['t_intern_ms'])):
        ...
        if x_lsoda_data != 0 and y_bdf_data != 0:
            if x_lsoda_data > y_bdf_data:
                bdf_lsoda_x.append(x_lsoda_data)
                bdf_lsoda_y.append(y_bdf_data)
                ratio_lsoda_bdf.append(np.log10(x_lsoda_data / y_bdf_data))
                num_states_lsoda_bdf.append(states_data)
            elif y_bdf_data > x_lsoda_data:
                lsoda_bdf_x.append(x_lsoda_data)
                lsoda_bdf_y.append(y_bdf_data)
                ratio_bdf_lsoda.append(np.log10(x_lsoda_data / y_bdf_data))
                num_states_bdf_lsoda.append(states_data)
            elif x_lsoda_data == y_bdf_data:
                equal_x.append(x_lsoda_data)
                equal_y.append(y_bdf_data)
                ratio_equal.append(np.log10(x_lsoda_data / y_bdf_data))
                num_states_equal.append(states_data)

        elif x_lsoda_data == 0 and y_bdf_data != 0:
            lsoda_zero_x.append(300000)
            lsoda_zero_y.append(y_bdf_data)
            ratio_lsoda_zero.append(np.log10(1000))
            num_states_lsoda_zero.append(states_data)

        elif x_lsoda_data != 0 and y_bdf_data == 0:
            bdf_zero_x.append(x_lsoda_data)
            bdf_zero_y.append(300000)
            ratio_bdf_zero.append(-np.log10(1000))
...
print('lsoda_bdf_x_smallest: ' + str(sorted(lsoda_bdf_x)[0]))
print('lsoda_bdf_y_smallest: ' + str(sorted(lsoda_bdf_y)[0]))
print('bdf_lsoda_x_smallest: ' + str(sorted(bdf_lsoda_x)[0]))
print('bdf_lsoda_y_smallest: ' + str(sorted(bdf_lsoda_y)[0]))
...
print('lsoda_bdf_x_largest: ' + str(sorted(lsoda_bdf_x, reverse=True)[0]))
print('lsoda_bdf_y_largest: ' + str(sorted(lsoda_bdf_y, reverse=True)[0]))
print('bdf_lsoda_x_largest: ' + str(sorted(bdf_lsoda_x, reverse=True)[0]))
print('bdf_lsoda_y_largest: ' + str(sorted(bdf_lsoda_y, reverse=True)[0]))
...
print('ratio_bdf_lsoda_largest: ' + str(sorted(ratio_bdf_lsoda, reverse=True)[0]))
print('ratio_bdf_lsoda_samllest: ' + str(sorted(ratio_bdf_lsoda)[0]))
print('ratio_lsoda_bdf_largest: ' + str(sorted(ratio_lsoda_bdf, reverse=True)[0]))
print('ratio_lsoda_bdf_smallest: ' + str(sorted(ratio_lsoda_bdf)[0]))
...
# grey
grid_grey = np.vstack([np.log(lsoda_bdf_x), np.log(lsoda_bdf_y)])
kde_grey = gaussian_kde(grid_grey)(grid_grey)
# blue
grid_blue = np.vstack([np.log(bdf_lsoda_x), np.log(bdf_lsoda_y)])
kde_blue = gaussian_kde(grid_blue)(grid_blue)

# Sort the points by density, so that the densest points are plotted last
# grey
ids_grey = kde_grey.argsort()
lsoda_bdf_x, lsoda_bdf_y, kde_grey = np.array(lsoda_bdf_x)[ids_grey], np.array(lsoda_bdf_y)[ids_grey], np.array(kde_grey)[ids_grey]
# blue
ids_blue = kde_blue.argsort()
bdf_lsoda_x, bdf_lsoda_y, kde_blue = np.array(bdf_lsoda_x)[ids_blue], np.array(bdf_lsoda_y)[ids_blue], np.array(kde_blue)[ids_blue]
...
z = range(0, 300000)
plt1 = ax.scatter(lsoda_bdf_x, lsoda_bdf_y, s=marker_size, c=kde_grey, cmap=cm_1, label='LSODA faster: ' + str(round(len(lsoda_bdf_x) / len(lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %', zorder=10, clip_on=False, alpha=alpha)
plt2 = ax.scatter(bdf_lsoda_x, bdf_lsoda_y, s=marker_size, c=kde_blue, cmap=cm_2, label='BDF faster: ' + str(round(len(bdf_lsoda_x)/len(lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %', zorder=10, clip_on=False, alpha=alpha)
...
plt5 = ax.scatter(bdf_zero_x, bdf_zero_y, c='blue', cmap=cm_1, s=marker_size, facecolors='none', edgecolors='#4c3340', marker='D', zorder=10, clip_on=False)
...
ax.set_ylabel('BDF simulation time [ms]', labelpad=-5, fontsize=fontsize)
...
ax.text(0.6, 80000, 'LSODA faster: ' + str(round(len(lsoda_bdf_x) / len(lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %', fontsize=fontsize)
ax.text(0.6, 30000, 'BDF faster: ' + str(round(len(bdf_lsoda_x)/len(lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %', fontsize=fontsize)
...
ax.text(100, 400000, 'only BDF failed: ' + str(round(len(bdf_zero_x) / len(
    lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %',
...
# plot data
plt.scatter(ratio_lsoda_bdf, num_states_lsoda_bdf,
            s=marker_size, c=kde_blue, cmap=cm_2, alpha=alpha,
            zorder=10, clip_on=False)
plt.scatter(ratio_bdf_lsoda, num_states_bdf_lsoda,
...
ax2.set_xlabel('Computation time ratio LSODA / BDF', fontsize=fontsize)
plt.xticks([-np.log10(1000), -2, -1, 0, 1, 2, np.log10(1000)],
           ['$10^{-3}$', '$10^{-2}$', '$10^{-1}$', '1', '$10^1$', '$10^2$',
            '$10^3$'], fontsize=fontsize)
ax2.text(-np.log10(1500), 20, 'BDF failed', fontsize=fontsize, rotation=90, ha='center')
